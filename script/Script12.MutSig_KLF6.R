...
sampleslist <- sampleslist[!grepl("MetNa22T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetNa07T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetNa06T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret08T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret16T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret17T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret21T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret22T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret23T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret24T", sampleslist)]
sampleslist <- sampleslist[!grepl("MetPret26T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS32T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS35T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS36T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS37T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS38T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS39T", sampleslist)]
sampleslist <- sampleslist[!grepl("RAS40T", sampleslist)]
...
all_genes <- suppressMessages(genes(txdb, single.strand.genes.only = FALSE))

# Haal de gen regio op via de Entrez-ID "
...
gene <- all_genes[which(names(all_genes) == entrez)]
gene 

# Zoek voor elk element in je VCF_GRList naar overlaps met de KRAS-regio
mutations_list <- lapply(VCF_GRList, function(gr) {
  ov <- findOverlaps(gr, gene, ignore.strand = TRUE)
  gr[queryHits(ov)]
})

# Combineer de resultaten in één GRanges-object
mutations_all <- do.call(c, mutations_list)
mutations_all
