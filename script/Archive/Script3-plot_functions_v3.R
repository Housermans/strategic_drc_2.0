...
plot_per_condition <- function(df, condition, fitcurves = TRUE, custom_colors=NULL) {
...
plot_per_condition_factorial <- function(df, condition, colorby="chemo_naive", fitcurves = TRUE, custom_colors = NULL) {
  df <- df[df$condition == condition, ]
  title <- condition
...
plot_multiple <- function(exp_name, select_on="Passed_QC", selection_value=1, colorby="chemo_naive", color_numeric=FALSE, fitcurves=TRUE, WGS=FALSE, OPTIC=FALSE, OPTIC_only=TRUE, custom_colors=NULL) {
  ...
  for (condition in unique(d$condition)) {
    ...
    if (color_numeric) {
      q <- plot_per_condition_numeric(d, condition, colorby=colorby, fitcurves=fitcurves, custom_colors=custom_colors)
    } else {
...
adjuvant_colors <- c("#00FF00", "#00FFFF", "#FF0000")


plot_selected_conditions("adjuvant1", select_on="Passed_QC", fitcurves = FALSE, WGS=TRUE, colorby = "Adjuvant_chemo_naive", selected_conditions = c("5-FU", "Oxaliplatin", "SN-38"), custom_colors=adjuvant_colors)
...
# plot_selected_conditions("response_SOC_chemo_naive1", select_on="Passed_QC", fitcurves = FALSE, WGS=TRUE, colorby = "response_oxaliplatin_chemo_naive", selected_conditions = c("Oxaliplatin"), custom_colors=three_colors)
plot_selected_conditions("response_SOC_chemo_naive1", select_on="Passed_QC", fitcurves = FALSE, WGS=TRUE, colorby = "response_SN-38_chemo_naive", selected_conditions = c("SN-38"), custom_colors=four_colors)
