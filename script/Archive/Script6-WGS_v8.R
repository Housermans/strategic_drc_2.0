...
make_boxplot <- function(data, x_var, y_var, comparisons, x_labels = NULL, x_lab = NULL, y_lab = NULL, label_y = 1.1, facet=TRUE) {
  ...
  if (facet){
    plot <- plot + facet_grid(. ~ condition)}
  return(plot)
}

# Function to create a correlation plot
make_corplot <- function(data, x_var, y_var, x_lab = NULL, y_lab = NULL, annotate_stats = FALSE, facet=TRUE) {
  ...
  if (facet){
    plot <- plot + facet_grid(. ~ condition)}
  if (annotate_stats) {
    cor_test <- cor.test(data[[x_var]], data[[y_var]], method = "pearson")
    r_value <- round(cor_test$estimate, 2)
    p_value <- cor_test$p.value
    plot <- plot + annotate("text", x = Inf, y = Inf, label = paste0("r = ", r_value, "\n", "p = ", round(p_value, 4)),
...
corplots <- lapply(WGS_variable, function(var) {
  make_corplot(PDO_WGS_screen, x_var = "AUC", y_var =  sprintf("log(%s)", var), 
               x_lab = "AUC", y_lab = paste0(var, "(log)"))
})
...
pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "corplots_karyotype"), width = 19, height = 2*length(WGS_variable_karyotype)) 
...
pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "boxplots_signatures_pretreated"), width = 19, height = 2 * length(signature_variables))
plots_signature <- lapply(signature_variables, function(var) {
  make_boxplot(PDO_WGS_screen_pretreatedonly, x_var = var, y_var = "AUC", 
               comparisons = list(c("present", "absent")), x_lab = var, y_lab = "AUC", label_y = 0.9)
})
grid.arrange(grobs = plots_signature, nrow = length(signature_variables))
dev.off()

PDO_WGS_screen_pretreatedonly$org_name[PDO_WGS_screen_pretreatedonly$DBS5 == 0] #RAS05 heeft geen DBS5, niet goed gecalld?

# Generate boxplots for deep deletions in chemonaive group
PDO_WGS_screen %>%
...
make_boxplot(PDO_WGS_screen_chemonaivefiltered, x_var = "has_deep_deletion", y_var = "AUC", 
             comparisons = list(c("present", "absent")), x_lab = "has_deep_deletion", y_lab = "AUC", label_y = 0.9)

# Generate boxplots for adjuvant treatment in chemonaive group
make_boxplot(PDO_WGS_screen_chemonaivefiltered, x_var = "Adjuvant_received", y_var = "AUC", 
             comparisons = list(c("Yes", "No")), x_lab = "Adjuvant", y_lab = "AUC", label_y = 0.9)

# Generate boxplots for signature variables (all samples)
pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "boxplots_signatures"), width = 19, height = 2 * length(signature_variables))
plots_signature <- lapply(signature_variables, function(var) {
  make_boxplot(PDO_WGS_screen, x_var = var, y_var = "AUC", 
               comparisons = list(c("present", "absent")), x_lab = var, y_lab = "AUC", label_y = 0.9)
})
...
# FIRST TURN OFF FACET GRID IN PLOT FUNCIONS !!!!!!!!!!!!!!!!
...
boxplots_SBS17a <- lapply(WGS_variable, function(var) {
  make_boxplot(mat_HMF_treatment, x_var = "SBS17a_cat", y_var = var,
               comparisons = list(c("absent", "present")), x_lab = "signature", y_lab = var, facet=FALSE)})
