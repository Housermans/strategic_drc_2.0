...
select_files_and_add_metadata <- function(select_on = "Analyse", plot_condition = "all", selection_value = 1, metadata_fac=c("chemo_naive", "RASTRIC", "Passed_QC", "Tween_bad", "DMSO_bad", "Clin_benefit", "Analyse", "Analyse2", "OPTIC"), metadata_num=c("RAS_pct_change", "org_id"), AUC=FALSE, WGS=FALSE, OPTIC=FALSE, OPTIC_only=FALSE) {
  ...
  status <- Analyse %>% dplyr::select(STR_ID, org_name, org_id, all_of(metadata_fac), all_of(metadata_num))
...
plot_per_condition <- function(df, condition, fitcurves = TRUE) {
...
plot_per_condition_factorial <- function(df, condition, colorby="chemo_naive", fitcurves = TRUE) {
  df <- df[df$condition == condition, ]
  title <- condition
...
plot_per_condition_numeric <- function(df, condition, colorby="RAS_pct_change", fitcurves = TRUE) {
  df <- df[df$condition == condition, ]
  title <- condition
...
plot_multiple <- function(exp_name, select_on="Passed_QC", selection_value=1, colorby="chemo_naive", color_numeric=FALSE, fitcurves=TRUE, WGS=FALSE, OPTIC=FALSE, OPTIC_only=TRUE) {
  ...
  for (condition in unique(d$condition)) {
    ...
    if (color_numeric) {
      ...
      q <- plot_per_condition_factorial(d, condition, colorby=colorby, fitcurves=fitcurves)
    }
    p_and_q <- p + q
...
plot_single <- function(exp_name, condition, select_on="Analyse", selection_value=1, colorby="chemo_naive", color_numeric=FALSE, fitcurves=TRUE, WGS=FALSE, OPTIC=FALSE, OPTIC_only=FALSE) {
  if (!dir.exists(file.path(plot_output, exp_name))) {
    dir.create(file.path(plot_output, exp_name))
  }
...
plot_selected_conditions <- function(exp_name, select_on="Passed_QC", selection_value = 1, colorby="chemo_naive", selected_conditions=c("5-FU", "Oxaliplatin"), color_numeric=FALSE, fitcurves=TRUE, WGS=WGS, OPTIC=OPTIC, OPTIC_only=FALSE) {
  for (condition in selected_conditions) {
    plot_single(exp_name = exp_name, condition = condition, select_on=select_on, selection_value=selection_value, colorby=colorby, color_numeric=color_numeric, fitcurves=fitcurves, WGS=WGS, OPTIC=OPTIC, OPTIC_only = OPTIC_only)
  }
}

dont_plot_selected_conditions <- function(exp_name, select_on="Passed_QC", selection_value = 1, colorby="chemo_naive",  unselected_conditions=c("5-FU", "Oxaliplatin"), color_numeric=FALSE, fitcurves=fitcurves) {
  d <- select_files_and_add_metadata(select_on = select_on, selection_value = selection_value, WGS=WGS, OPTIC=OPTIC, OPTIC_only = OPTIC_only)
  for (condition in unique(d$condition)) {
    if (!condition %in% unselected_conditions) {
      plot_single(exp_name = exp_name, condition = condition, select_on=select_on, selection_value=selection_value, colorby=colorby, color_numeric=color_numeric, fitcurves=fitcurves)
    }
  }
}
...
#plot_multiple("PassedQC6_nonfit", select_on="Passed_QC", fitcurves = FALSE)
