...
make_figure_1_heatmap <- function(exp_name, data=fig1_df, selection=scurve, selection_name="figure1plot") {
  ...
  pheatmap_AUC2 <- function(data = data_reshape2, title_plot = selection_name) {
    ...
    heatmap_annotation <- data  %>%
      dplyr::select(Treatment_status, Palliative_duration_category, Adjuvant_received.y, origin_location, MAPK_status, Sidedness, OS_category) %>%
      dplyr::rename(`Treatment status` = Treatment_status, 
    ...
    soc <- ifelse(colnames(data_reshape) %in% c("5-FU", "Oxaliplatin", "SN-38"), "Standard of Care", "Experimental treatment")
...
make_figure_1_heatmap(exp_name_current, selection_name="heatmap_figure1_final_OSupdate_colsplit")
