...
metrics_plot_dir <- file.path(metrics_dir, "230912_QC")
exp_name <- "230912_QC"
                     ...
                     normalize=TRUE) 
{
  ...
  if (selected_conditions == "all") {
    selected_conditions <- c("5-FU", "vi_bi_la", "Oxaliplatin", "Lapatinib", "Binimetinib", "Alpelisib", "Navitoclax", "CHEK1", "SN-38" ,
          "binimetinib_lapatinib", "SN38_CHEK1", "alpelisib_lapatinib", "Vinorelbine", "binimetinib_alpelisib", "navitoclax_vinorelbine")
  }  
...
df <- save_metrics(exp_name)
make_boxplots(exp_name, df)

norm_df2 <- read_excel(file.path(metrics_plot_dir, paste0(exp_name, "_metrics_normalized.xlsx")))

convert_long_to_wide <- function(df, metric) {
  wide_df <- df %>%
    pivot_wider(
      id_cols = org_name,  
  ...
  write.xlsx(wide_df, file.path(metrics_plot_dir, paste0(exp_name, "AUClow_wide.xlsx")))
  View(wide_df)
}

convert_long_to_wide(norm_df2, AUC_low)
...
make_heatmap("230912_QC", norm_df2)
