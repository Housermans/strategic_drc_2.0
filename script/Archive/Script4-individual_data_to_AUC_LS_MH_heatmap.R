...
normalize_df <- function(df) {
  metrics_df_norm <- df
  metrics_df_norm = metrics_df_norm %>% dplyr::select(org_name,condition,chemo_naive,RAS_pct_change,Clin_benefit,AUC,GRmax,AUC_low,GRmax_low,GR50,xmid)
  
  #normalize per condition IN PROGRESS
  for(i in 6:ncol(metrics_df_norm)) {       
  ...
  metrics_df_norm <- metrics_df_norm %>%
    group_by(condition) %>%
    mutate_at(vars(6:last_col()), list(~(. - min(., na.rm = TRUE)) / (max(., na.rm = TRUE) - min(., na.rm = TRUE))))
                     ...
                     normalize=TRUE) 
{
  ...
  for (condition in selected_conditions) {
    ...
    cod_individual <- select_files_and_add_metadata(select_on = select_on, selection_value = selection_value, individual_reps=TRUE)
    ...
    metric <- d %>% dplyr::select(conc_condition, mean_GR, org_name, condition, chemo_naive, RAS_pct_change, Clin_benefit) %>%
    ...
    metric_lowconc <- d %>% dplyr::select(conc_condition, mean_GR, org_name, condition, chemo_naive, RAS_pct_change, Clin_benefit) %>%
      mutate(mean_GR_positive=(mean_GR +1)/2)  %>% #GR range 0-1 for calculating AUC
    ...
    metadata <- d %>% dplyr::select(org_name, RAS_pct_change, Clin_benefit) %>%
      group_by(org_name, RAS_pct_change, Clin_benefit) %>%
      summarise()
    ...
    #merge dataframe non-fitted metrics with fitted metrics
    metric_metriclowconcmeta <- merge(metric_metriclowconc, metadata,by="org_name")
    
    #merge dataframe non-fitted metrics with fitted metrics
    dataframe_metrics <- merge(metric_metriclowconcmeta, dataframe_fit_metrics, by="org_name")
...
make_boxplots <- function(exp_name, data){
  boxplot_AUC_low <- ggplot(data, aes(y=AUC_low, x=chemo_naive, color = Clin_benefit)) + 
  ...
  boxplot_AUC <- ggplot(data, aes(y=AUC, x=chemo_naive, color = Clin_benefit)) + 
    facet_grid(~condition) +
    geom_boxplot(size =0.6, outlier.shape = NA) +
    geom_jitter(position=position_jitter(0.1))+
    #stat_summary(fun = mean, geom = "text", col = "red", vjust = +2, aes(label = paste("Mean:", round(..y.., digits = 2))))+
  ...
  boxplot_GRmax_low <- ggplot(data, aes(y=GRmax_low, x=chemo_naive, color = Clin_benefit)) + 
    facet_grid(~condition) +
    geom_boxplot(size =0.6, outlier.shape = NA) +
    geom_jitter(position=position_jitter(0.1))+
    #stat_summary(fun = mean, geom = "text", col = "red", vjust = +2, aes(label = paste("Mean:", round(..y.., digits = 2))))+
  ...
  boxplot_GRmax <- ggplot(data, aes(y=GRmax, x=chemo_naive, color = Clin_benefit)) + 
    facet_grid(~condition) +
    geom_boxplot(size =0.6, outlier.shape = NA) +
    geom_jitter(position=position_jitter(0.1))+
    #stat_summary(fun = mean, geom = "text", col = "red", vjust = +2, aes(label = paste("Mean:", round(..y.., digits = 2))))+
  ...
  boxplot_GR0 <- ggplot(data, aes(y=GR50, x=chemo_naive, color = Clin_benefit)) + 
    facet_grid(~condition) +
    geom_boxplot(size =0.6, outlier.shape = NA) +
    geom_jitter(position=position_jitter(0.1))+
    #stat_summary(fun = mean, geom = "text", col = "red", vjust = +2, aes(label = paste("Mean:", round(..y.., digits = 2))))+
...
make_heatmap <- function(exp_name, data){
  data_select = data %>% dplyr::select("org_name", "condition", "chemo_naive", "AUC", "RAS_pct_change")
  data_select <- data_select %>% distinct(org_name, condition, chemo_naive, RAS_pct_change, AUC, .keep_all = TRUE)
  ...
  heatmap_annotation <- data_reshape2[["RAS_pct_change"]]
  data_reshape2$RAS_pct_change <- NULL
  
  data_reshape2 <- data_reshape2[complete.cases(data_reshape2), ]
  
  # heatmap_annotation <- heatmap_annotation %>% 
  #   rename(
  #     "Chemo naive" = chemo_naive)
  # data_reshape2$chemo_naive <- NULL
  pheatmap <- pheatmap(data_reshape2,
  ...
  tiff(file = file.path(metrics_dir, exp_name, "AUC_heatmap.tiff"),
       units="in",
       width = (7),
       height = (7),
       res=300)
  ...
  heatmap_annotation <- data_reshape2[ c("RAS_pct_change")]
  data_reshape2$RAS_pct_change <- NULL
  ...
  tiff(file = file.path(metrics_dir, exp_name, "AUC_low_heatmap.tiff"),
       units="in",
       width = (7),
       height = (7),
       res=300) 
...
df <- save_metrics("RASTRIC_AUCs", select_on = "Analyse2", normalize = FALSE)
norm_df <- normalize_df(df)
make_boxplots("RASTRIC_AUCs", norm_df)
make_heatmap("RASTRIC_AUCs", norm_df)
