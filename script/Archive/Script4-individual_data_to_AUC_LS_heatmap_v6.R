...
metrics_normalized_dir <- file.path(metrics_dir, "240605_QC")
...
df <- save_metrics("240605_QC")
make_boxplots("240605_QC", df)
#make_boxplots("240605_QC", norm_df2)
norm_df2 <- read_excel(file.path(metrics_normalized_dir, "240605_QC_metrics_normalized.xlsx")) #if you do not want to run the full script
...
make_heatmap("240605_QC", norm_df2, selection=scurve, selection_name = "scurve")
make_heatmap("240605_QC", norm_df2, selection=singles, selection_name = "singles")
make_heatmap("240605_QC", norm_df2, selection=chemo, selection_name = "chemo")
make_heatmap("240605_QC", norm_df2, selection=FU_ox, selection_name = "5-FU & ox")
make_heatmap("240605_QC", norm_df2, selection=response_expected, selection_name = "response_expected")
make_heatmap("240605_QC", norm_df2, selection=targeted, selection_name = "targeted")
...
corplot <- ggplot(wide_df, aes(x = AUC_OPTIC, y = AUC_STRATEGIC, color = org_name_clean)) +
  ...
  theme_classic()
ggsave(file = file.path(metrics_dir, "240605_QC", "corplot_AUC.pdf"), plot = corplot, width=9, height = 3)
