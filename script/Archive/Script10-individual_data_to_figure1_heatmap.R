...
make_figure_1_heatmap <- function(exp_name, data=fig1_df, selection=scurve, selection_name="figure1plot") {
  ...
  data_select <- data %>% 
    ...
    dplyr::select(sampleId, condition, `Sample type.y`, chemo_naive, AUC, Sidedness, MAPK_status, origin_location, Palliative_duration_category, Adjuvant_received.y, OS_category) %>% 
  ...
  pheatmap_AUC2 <- function(data = data_reshape2, title_plot = selection_name) {
    ...
    heatmap_annotation <- data  %>%
      dplyr::select(`Sample type.y`, Palliative_duration_category, Adjuvant_received.y, origin_location, MAPK_status, Sidedness, OS_category) %>%
...
make_figure_1_heatmap(exp_name_current, selection_name="heatmap_figure1_finalxx")
