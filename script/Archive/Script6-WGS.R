...
boxplot_tmb <- ggplot(purple_treatment, aes(y = svTumorMutationalBurden, x = chemotherapy)) + 
  geom_jitter(width = 0.2, size=0.5) +
  labs(title = "", y = "tmb", x = "chemotherapy") + 
  theme_classic()+
  scale_y_continuous(trans = "log10")+
  stat_compare_means(method = "t.test", label = "p.format", label.y = 1)+
  stat_summary(fun.data = mean_cl_normal, geom = "crossbar", width = 0.5)
...
# Plonumeric# Plot heatmap with legend
pheatmap(df, legend_labels = )
...
#save plot
dirpath=WGS_plot_dir
plot_name="drivers_and_supressors_HMF"
pdf(sprintf("%s/%s.pdf",dirpath, plot_name) , useDingbats = F, width = 17, height = 4)
plot(plot)
dev.off()

#deduplication plot
test <- purple %>% dplyr::select(wholeGenomeDuplication)
row.names(test) <- purple$sampleId
labels_purple <- colnames(test)
names(test)<-seq(1, ncol(test), by=1)
m <- as.matrix(test)
dt <- data.table(reshape2::melt(m))
dt <- dt[, strsplit(as.character(value), "/"), by=list(Var1, Var2)]  # this expands "X/Y/Z" into three rows
dt[, shift:=(1:(.N))/.N - 1/(2 * .N) - 1/2, by=list(Var1, Var2)]
...
ggplot() + 
  geom_tile(data = dt,aes(Var1,y=test, fill=V1, height=height),
            color="grey", size=0.1) + xlab('sample ID') + ylab('')+
  scale_y_continuous(breaks = seq(1, ncol(test), by=1), labels = labels_purple)+
  guides(fill=guide_legend(title="Effect"))+
