...
metrics_normalized_dir <- file.path(metrics_dir, "240104_QC")
...
df <- save_metrics("240104_QC")
make_boxplots("230912_QC", df)

norm_df <- normalize_df(df)

norm_df2 <- read_excel(file.path(metrics_normalized_dir, "240104_QC_metrics_normalized.xlsx")) #if you do not want to run the full script
...
make_heatmap <- function(exp_name, data){
  ...
  heatmap_annotation <- data_reshape2[ c("chemo_naive", "STR_ID")]
...
make_heatmap("240104_QC", norm_df2)
