...
# Make boxplots for TMB/TML/WG disruptions
...
# Prep merged (combined) data for table with driver genes
...
metrics_dir <- paste0(home_dir, "/Analyse 2.0/6_metrics/240516_QC")

mat_PDO <- read.csv(file.path(Arne_driver_dir, "Driveroverview_rastric.txt"), sep = "\t")
metrics <- read_xlsx(file.path(metrics_dir, "240516_QC_metrics_normalized.xlsx"))
...
purple_PDO <- read.csv(file.path(WGS_dir, "organoid_purple_purity_data_prefilter.tsv"), sep = "\t")   
categorized_columns <- read.csv(file.path(WGS_plot_dir, "categorized_columns.csv"), sep = ",")   
...
mat_HMF_treatment <- merge(mat_HMF_treatment, purple_PDO, by = "sampleId") #merge meta data and WGS PDO data
...
names(mat_HMF_treatment)[names(mat_HMF_treatment) == "Organoid line"] <- "org_name"
...
no_WGS <- anti_join(filtered_metrics, mat_HMF_treatment, by = "org_name") 
unique(no_WGS$org_name) #"HUB-02-C2-89" "OPT0426"      "OPT0502"      "RAS06"        "RAS34"    
no_drugscreen <- anti_join(mat_HMF_treatment, filtered_metrics, by = "org_name")
unique(no_drugscreen$org_name) #"OPT0031" "OPT0032" "OPT0044" "OPT0404" "OPT0431" "OPT0048" "OPT0119" "OPT0418

PDO_WGS_screen <- merge(filtered_metrics, mat_HMF_treatment, by = "org_name") #merge WGS & drugscreen data

#make 1 variable for presence of deep deletions 
...
columns_to_check2 <- c("FHIT","WWC3")
PDO_WGS_screen$FHIT_WWC3_deletion <- apply(PDO_WGS_screen[, columns_to_check2], 1, function(row) {
  if ('deep deletion' %in% row) {
    return('yes')
  } else {
    return('no')
...
PDO_WGS_screen_pretreatedonly <- PDO_WGS_screen %>%
  mutate(across(contains(c("SBS", "DBS")), ~ ifelse(chemo_naive == "yes", NA, .)))
...
signature_variable <- c("SBS17_cat", "SBS17a_cat", "SBS17b_cat", "SBS17_threshold")
...
unique(columns_with_deep_deletion)

PDO_WGS_screen_pretreatedfiltered$FHIT_WWC3_deletion
boxplot_deepdeletion_pretreated <- ggplot(PDO_WGS_screen_pretreatedfiltered, aes(x = FHIT_WWC3_deletion, y = AUC)) +
    geom_jitter(width = 0.2, size = 1, aes(color = FHIT_WWC3_deletion)) +
    facet_grid(~condition) +
    labs(title = "", y = "AUC", x = "deep deletion") + 
    theme_classic() +
    ylim(-0.1, 1.3) +
    #scale_color_manual(values = c("NonNA_value1" = "color1", "NonNA_value2" = "color2")) +
    stat_summary(fun.data = mean_cl_normal, geom = "crossbar", width = 0.5) +
    stat_compare_means(comparisons = list(c("yes", "no")), method = "t.test", 
                       aes(label = ..p.signif..),  p.adj = "none", label.y = 1.1)

pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "boxplots_deepdeletions_pretreated_FHITWWC3"), width = 19, height = 2)
boxplot_deepdeletion_pretreated
dev.off()
