...
metrics_normalized_dir <- file.path(metrics_dir, "230620_QC_normalized")
                     ...
                     normalize=TRUE) 
{
  ...
  for (condition in selected_conditions) {
    ...
    metric_lowconc <- d %>% dplyr::select(conc_condition, mean_GR, org_name,STR_ID, condition, chemo_naive) %>%
      mutate(mean_GR_positive=(mean_GR +1)/2)  %>% #GR range 0-1 for calculating AUC
...
df <- save_metrics("230912_QC")
make_boxplots("230912_QC", df)

norm_df <- normalize_df(df)

norm_df2 <- read_excel(file.path(metrics_normalized_dir, "230620_QC_normalized_metrics_normalized.xlsx")) #if you do not want to run the full script
...
make_heatmap <- function(exp_name, data){
  data_select = data %>% dplyr::select("org_name","STR_ID", "condition", "chemo_naive", "AUC")
  data_select <- data_select %>% distinct(org_name,STR_ID, condition, chemo_naive, AUC, .keep_all = TRUE)
  data_reshape2 <- spread(data_select,                                  
                          key = condition,
                          value = AUC)  
    data_reshape2 <- as.data.frame(data_reshape2)
  rownames(data_reshape2) <- data_reshape2$STR_ID #reassigning row names
  ...
  data_select = data %>% dplyr::select("org_name","STR_ID", "condition", "chemo_naive", "AUC_low")
  data_select <- data_select %>% distinct(org_name,STR_ID, condition, chemo_naive, AUC_low, .keep_all = TRUE)
  data_reshape2 <- spread(data_select,                                  
                          key = condition,
                          value = AUC_low)  
  data_reshape2 <- as.data.frame(data_reshape2)
...
make_heatmap("230620_QC", norm_df2)

exp_name <- "230620_QC_normalized"
data <- norm_df2
