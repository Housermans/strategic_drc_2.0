...
metrics_normalized_dir <- file.path(metrics_dir, "240516_QC")
                     ...
                     normalize=TRUE) 
{
  ...
  for (condition in selected_conditions) {
    ...
    concentration_filters2 <- list(
      Alpelisib = c(0.57420, 1.88100, 6.13800, 20.00000),#3 laagste
      Lapatinib = c(0.57420, 1.88100, 6.13800, 20.00000), #3 laagste
      lapatinib_alpelisib = c(0.57420, 1.88100, 6.13800, 20.00000)#3 laagste
    )
...
df <- save_metrics("240516_QC")
make_boxplots("240516_QC", df)
#make_boxplots("240516_QC", norm_df2)
norm_df2 <- read_excel(file.path(metrics_normalized_dir, "240516_QC_metrics_normalized.xlsx")) #if you do not want to run the full script
...
scurve <- c("5-FU", "vi_bi_la", "Oxaliplatin", "Navitoclax", "CHEK1", "SN-38", "binimetinib_lapatinib")

make_heatmap <- function(exp_name, data, selection=NULL, selection_name){
  selection <- selection
...
make_heatmap("240516_QC", norm_df2, selection=scurve, selection_name = "scurve")
make_heatmap("240516_QC", norm_df2, selection=singles, selection_name = "singles")
make_heatmap("240516_QC", norm_df2, selection=chemo, selection_name = "chemo")
make_heatmap("240516_QC", norm_df2, selection=FU_ox, selection_name = "5-FU & ox")
make_heatmap("240516_QC", norm_df2, selection=response_expected, selection_name = "response_expected")
make_heatmap("240516_QC", norm_df2, selection=targeted, selection_name = "targeted")
...
corplot <- ggplot(wide_df, aes(x = AUC_OPTIC, y = AUC_STRATEGIC, color = org_name_clean)) +
  ...
  theme_classic()
ggsave(file = file.path(metrics_dir, "240516_QC", "corplot_AUC.pdf"), plot = corplot, width=9, height = 3)
