...
# Categories karyotype
categorize_karyotype <- function(x) {
  ifelse(x == 2, "diploid", 
         ifelse(x < 2, "loss", "gain"))
}

categorized_karyotype <- as.data.frame(lapply(PDO_WGS_screen[WGS_variable_karyotype], categorize_karyotype))
other_columns <- PDO_WGS_screen[, !startsWith(colnames(PDO_WGS_screen), "chr")]
PDO_WGS_screen_karyotype_categorized <- cbind(other_columns, categorized_karyotype)
WGS_variable_karyotype_categorized <- colnames(PDO_WGS_screen_karyotype_categorized)[startsWith(colnames(PDO_WGS_screen_karyotype_categorized), "chr")]

# Generate boxplots and corplots for karyotype
pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "corplots_karyotype"), width = 2*19, height = 2*length(WGS_variable_karyotype_categorized)) 
boxplots_karyotype <- lapply(WGS_variable_karyotype_categorized, function(var) {
  make_boxplot(PDO_WGS_screen_karyotype_categorized, y_var = "AUC", x_var = var, 
               y_lab = "AUC", x_lab = paste0(var, "(log)"), comparisons = list(c("diploid", "gain")))
})
grid.arrange(grobs = c(boxplots_karyotype), nrow = length(WGS_variable_karyotype))
dev.off()
...
pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "boxplots_signatures"), width = 2*19, 
...
corplots_signature_cont <- lapply(signature_variables_cont, function(var) {
  make_corplot(PDO_WGS_screen, x_var = "AUC", y_var = var, 
               x_lab = "PDO", y_lab = var, annotate_stats = TRUE)
})
...
pdf(sprintf("%s/%s.pdf", WGS_plot_dir, "boxplots_signatures_top5"), width = 2*19, 
    height = 2 * length(signature_variables)+2*length(signature_variables_cont))
plots_signature <- lapply(signature_variables, function(var) {
  make_boxplot(PDO_WGS_screen_top5_all, x_var = var, y_var = "AUC", 
               comparisons = list(c("present", "absent")), x_lab = var, y_lab = "AUC top 5", label_y = 0.9)
})
plots_signature_cont <- lapply(signature_variables_cont, function(var) {
  make_boxplot(PDO_WGS_screen_top5_all, x_var = "AUC_category", y_var = var, 
