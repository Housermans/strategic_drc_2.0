...
### VERSION CONTROL 
# Version 5 (27-05-2024): 
# added a case_when statement to the read_screen function that evaluates
# whether a screening is either G12C or halfscreen so it reads in only half an
# excel file. 
# It worked now in manual testing on examples of full screen, half screen and 
# G12C screens. 
...
read_screen <- function(exp_id, organoid_name) {
  ...
  halfscreen <- case_when(
    startsWith(pull(org_row, screen_setup), "G12Cscreen") ~ TRUE, 
    startsWith(pull(org_row, screen_setup), "HalfScreen") ~ TRUE, 
    .default = FALSE)
...
apply_viab <- function(value_corr, DMSO_status, Tween, DMSO) {
  if (is.na(DMSO_status)) {
    return(NA)
  }
  if (value_corr <= 0) {
    ...
    viab <- value_corr/DMSO
  } else if (DMSO_status == "N") {
    viab <- value_corr/Tween
  }
}
...
read_organoid <- function(exp_id, organoid_name) {
  ...
  org_df$viab <- NA
  for (i in 1:nrow(org_df)) {
    org_df[i,]$viab <- apply_viab(org_df[i,]$value_corr, org_df[i,]$DMSO_status, Tween, DMSO)
  }
  ...
  print(paste("writing to", file.path(org_data_dir, paste0(paste(date_short, exp_id, organoid_name, sep="_"), ".xlsx"))))
  org_df
}
...
# read_organoid(exp_id = "MHUI05", organoid_name = "RAS44_mhui4")
read_experiment("MHUI06")
#read_experiment("STR25")
#read_experiment("STRG12C2")
