...
readFile <- function(filename){
  ...
  sample_data <- read.table(filename, header=T, sep= "\t")
  return(sample_data)
}
...
chromArmPloidy <- function(sample_data, filename){
  majorPloidy_list <- c()
  minorPloidy_list <- c()
  lapply(paste("chr",c(1:22),sep=""), function(c){
    if(c %!in% c("chr13","chr14","chr15","chr21","chr22")){
...
optimalization <- function(ploidy_list, filename, tp){
  #ploidy_list  <- as.numeric(unlist(return_list[2]))
...
getChromArms <- function(){
  ...
  chromArms <- paste0("chr",chromArms)
...
ploidyEstimator <- function(sampleslist){
  ...
  return_list <- lapply(files, function(fileb){
    file_name_CN <- basename(fileb) %>% gsub(pattern = "\\..*$",replacement =  "")
    print(file_name_CN)
    sample_data <- readFile(fileb)

    sample_data <- addMajorMinorAllele(sample_data)
    return_list <- chromArmPloidy(sample_data, fileb)

    majorPloidyList <- optimalization(unlist(return_list[1]), file_name_CN, "major")
    minorPloidyList <- optimalization(unlist(return_list[2]), file_name_CN, "minor")
    print(majorPloidyList)
...
# Import all purple.cnv.somatic.tsv files in the WGS folder
dirs<-Sys.glob(file.path(WGS_analysis_files_arne2_dir))
sampleslist=list.files(dirs, pattern = "purple.cnv.somatic.tsv$",full.names=TRUE, recursive = TRUE)
length(sampleslist)
...
datalist2 <- datalist1 %>% tibble::rownames_to_column(var = "sampleId") %>% tidyr::separate(sampleId, c("sampleId", "mut_nr") )

setwd(WGS_plot_dir)
ploidyEstimator(sampleslist = sampleslist)
